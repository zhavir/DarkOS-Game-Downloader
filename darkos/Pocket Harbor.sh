#!/bin/sh
# DarkOS Tools launcher for the bundled Linux ARM64 executable.
set -eu

SCRIPT_DIR=$(CDPATH= cd "$(dirname "$0")" && pwd)
LAUNCHER="$SCRIPT_DIR/Pocket Harbor.sh"
APP_DIR="$SCRIPT_DIR/pocket-harbor"
EXECUTABLE="$APP_DIR/pocket-harbor"
LOG_FILE="$APP_DIR/pocket-harbor.log"
REFRESH_FILE="$APP_DIR/.refresh-emulationstation"
CA_BUNDLE="$APP_DIR/ca-certificates.crt"
UPDATE_DIR="$SCRIPT_DIR/.pocket-harbor-update"
UPDATE_BACKUP="$SCRIPT_DIR/.pocket-harbor-backup"
UPDATE_LAUNCHER_TEMP="$SCRIPT_DIR/.pocket-harbor-launcher.new"
PREVIOUS_LAUNCHER_NAME=".previous-launcher.sh"
FAILED_UPDATE_DIR="$SCRIPT_DIR/.pocket-harbor-failed"
VERIFICATION_MARKER=".update-awaiting-first-launch"

show_failure() {
    message="Pocket Harbor stopped with exit code $1.\n\nDiagnostic log:\n$LOG_FILE"
    if command -v dialog >/dev/null 2>&1; then
        dialog --clear --title "Pocket Harbor" --msgbox "$message" 12 64
        dialog --clear --title "Diagnostic log" --textbox "$LOG_FILE" 20 70
    else
        clear 2>/dev/null || true
        printf '\nPocket Harbor could not start.\n\n'
...
restore_previous_installation() {
    ...
    if [ ! -f "$UPDATE_BACKUP/pocket-harbor" ]; then
        return 1
    fi
    ...
    if [ -f "$FAILED_UPDATE_DIR/pocket-harbor.log" ]; then
        printf '%s\n' "Output captured from the failed updated version:" >>"$LOG_FILE"
        cat "$FAILED_UPDATE_DIR/pocket-harbor.log" >>"$LOG_FILE" 2>/dev/null || true
    fi
...
apply_pending_update() {
    ...
    staged_app="$UPDATE_DIR/pocket-harbor"
    staged_executable="$staged_app/pocket-harbor"
    staged_launcher="$UPDATE_DIR/Pocket Harbor.sh"
    ...
    if [ "$reported_version" != "ph $update_version" ]; then
        restore_previous_installation \
...
export PH_DOWNLOAD_DIR="${PH_DOWNLOAD_DIR:-$APP_DIR/.downloads}"
export PH_INSTALL_DIR="$APP_DIR"
export PH_ES_REFRESH_FILE="$REFRESH_FILE"
export PH_LOG_FILE="$LOG_FILE"
...
# openvt starts this internal mode on a real Linux virtual console. DarkOS normally
# starts Tools scripts with stdin/stdout detached from a TTY.
if [ "${1:-}" = "--run-on-vt" ]; then
    {
...
{
    printf 'Pocket Harbor diagnostic log\n'
