...
def detect_roms_directories(
    ...
    """Find all active dArkOS ROM partitions, including a dual-SD setup."""

    if configured is not None:
        return (configured,) if isinstance(configured, Path) else tuple(configured)

    return available_roms_directories((Path("/roms2"), Path("/roms")))
...
def available_roms_directories(candidates: Sequence[Path]) -> tuple[Path, ...]:
    ...
    # short console allow-list so customized dArkOS images are recognized.
    available = tuple(
        candidate
...
def detect_roms_directory(configured: Path | Sequence[Path] | None = None) -> Path | None:
    """Find the preferred dArkOS ROM partition for non-interactive use."""

    directories = detect_roms_directories(configured)
...
def move_to_arkos(
    ...
    if platform.arkos_folder is None:
        raise OrganizeError(f"{platform.name} does not have a supported dArkOS ROM folder.")
    existing_folder = next(
        (folder for folder in platform.arkos_folders if (roms_directory / folder).is_dir()),
