"""EmulationStation platform metadata and ROM-folder mappings."""

from collections.abc import Iterable, Sequence
from pathlib import Path

from ph.compatibility import is_unsupported_system
from ph.models import Platform
from ph.targets import LinuxTarget
...
# The initial folder profile follows DarkOS EmulationStation. Console
# families outside the supported handheld profile are intentionally omitted.
# Empty service codes use all-platform search while preserving the ROM destination.
DARKOS_PLATFORMS: tuple[Platform, ...] = (
...
def discover_platforms(
    roms_directories: Iterable[Path], known: Sequence[Platform] = DARKOS_PLATFORMS
) -> tuple[Platform, ...]:
...
def _build_aliases() -> dict[str, Platform]:
    ...
    for platform in DARKOS_PLATFORMS:
        values = (
            platform.name,
            platform.slug,
            platform.alias,
            platform.code,
            *platform.rom_folders,
        )
        for value in values:
            if value:
...
def platform_catalogue(target: LinuxTarget) -> tuple[Platform, ...]:
    """Return the ROM-folder catalogue owned by an OS target profile."""

    if target.platform_profile == "darkos":
        return DARKOS_PLATFORMS
    raise ValueError(f"No platform catalogue is registered for {target.display_name}.")


def resolve_platform(
    value: str,
    known: Sequence[Platform] = DARKOS_PLATFORMS,
) -> Platform | None:
    """Resolve a slug, code, short alias, or ROM folder case-insensitively."""

    normalized = value.strip().casefold()
    if known is DARKOS_PLATFORMS:
    ...
    return next(
        (
            ...
            in {
                ...
                platform.slug.casefold(),
                platform.alias.casefold(),
                platform.code.casefold(),
                *(folder.casefold() for folder in platform.rom_folders),
            }
        ),
        None,
    )
