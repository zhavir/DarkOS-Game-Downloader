"""Optional compatibility ratings and cached title matching."""
...
COMPATIBILITY_CATALOGUE_URL = "https://r36sgamelist.com"
...
# The upstream source assigns compatibility at console level. A title match
# confirms that the game is listed; actual performance can vary by device.
...
class CompatibilityError(RuntimeError):
    """The game compatibility catalogue could not be downloaded or stored."""
...
def normalize_console(value: str) -> str | None:
    """Map remote and ROM-folder console spellings to the compatibility catalogue."""
...
def is_unsupported_system(value: str) -> bool:
    """Return whether a console family is outside the supported handheld profile."""
...
class GameCompatibilityClient:
    """Fetch and cache the frontend-only game compatibility index."""

    def __init__(
        self,
        cache_path: Path,
        base_url: str = COMPATIBILITY_CATALOGUE_URL,
    ...
    def refresh(self) -> int:
        ...
        try:
            ...
            raise CompatibilityError(
                f"Could not update the compatibility catalogue: {error}"
            ) from error
        self._game_index = index
        LOGGER.info("Compatibility catalogue cached games=%d path=%s", len(index), self.cache_path)
        return len(index)
    ...
    def _write_cache(self, index: frozenset[GameKey], *, strict: bool = False) -> None:
        ...
        try:
            ...
            if strict:
                raise CompatibilityError(
                    f"Could not save the compatibility catalogue: {error}"
                ) from error
            LOGGER.warning("Could not cache the compatibility catalogue: %s", error)
