...
from ph.organizer import ROM_AND_SHARED_BIOS, ROM_LOCAL_BIOS
...
# ROM-folder/platform identifiers differ from RetroBIOS system identifiers.
...
class RetroBiosCatalog:
    ...
    def system_for(self, platform: Platform) -> BiosSystem | None:
        """Return the exact RetroBIOS system mapped to a local platform."""
    ...
    *,
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "pocket-harbor/retrobios",
        },
    )
    content = bytearray()
    try:
...
def install_bios(
    ...
    try:
        ...
        request = urllib.request.Request(
            url,
            headers={"User-Agent": "pocket-harbor/retrobios"},
        )
        with (
