...
For local use, set `PH_ROMS_DIR` to one test directory or `PH_ROMS_DIRS` to multiple directories
...
Search and downloads continue when the compatibility catalogue cannot be reached. Results then
show only known platform-level information or **Not listed**. Retry later to refresh the cached
catalogue.

The catalogue is downloaded only when it is missing. Use **Settings → Update compatibility catalogue**
...
DarkOS can configure another emulator core with different optional files. Open **Search and
...
- If the terminal type is unknown, try `TERM=xterm-256color uv run ph`.
...
```sh
PH_LOG_FILE=.local-test/pocket-harbor.log PH_LOG_LEVEL=DEBUG uv run ph
```
